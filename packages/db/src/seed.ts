...
async function main() {
  ...
  try {
    ...
    const catMap = insertedCategories.reduce(
      (acc, cat) => {
        acc[cat.name] = cat.id
        return acc
      },
    ...
    if (rahul && rtx4070) {
      const sale1 = await db
        ...
        .values({
          ...
          profit: (
            Number(rtx4070.sellingPrice) - Number(rtx4070.costPrice)
          ).toFixed(2),
    ...
    const techVision = insertedCustomers.find(
      (c) => c.name === "TechVision Solutions"
    )
    ...
    if (techVision && i7 && msiMobo && ssd) {
      const totalAmt =
        Number(i7.sellingPrice) * 2 +
        Number(msiMobo.sellingPrice) * 2 +
        Number(ssd.sellingPrice) * 2
      const costTotal =
        Number(i7.costPrice) * 2 +
        Number(msiMobo.costPrice) * 2 +
        Number(ssd.costPrice) * 2
